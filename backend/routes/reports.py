...
@reports_bp.route('/api/reports/customer-satisfaction', methods=['GET'])
@token_required
def get_customer_satisfaction():
    """客户满意度统计：按工单类型、负责人、月度趋势聚合满意度评分。"""
    payload = request.current_user
    role = payload['role']
    username = payload['username']
    ...
    owner_filter = "" if role in ('主任', '院长') else "AND (t.owner_id = ? OR t.created_by = ?)"
    owner_params = [] if role in ('主任', '院长') else [username, username]

    # 1. 总体满意度
    cursor.execute(f"""
        SELECT COUNT(*) as total,
               COALESCE(AVG(s.overall_score), 0) as avg_overall,
               COALESCE(AVG(s.response_speed), 0) as avg_response,
               COALESCE(AVG(s.attitude_score), 0) as avg_attitude,
               COALESCE(AVG(s.quality_score), 0) as avg_quality,
               SUM(CASE WHEN s.overall_score >= 4 THEN 1 ELSE 0 END) as satisfied
    ...
    satisfied = overall['satisfied'] or 0
    ...
    """, owner_params)
    by_type = []
    type_labels = {'consult': '咨询', 'complaint': '投诉', 'suggestion': '建议',
                   'fault': '故障', 'other': '其他'}
    ...
    if role in ('主任', '院长'):
        ...
        for r in cursor.fetchall():
            by_owner.append({
                'owner_name': r['owner_name'] or '未分配',
                'count': r['count'],
                'avg_score': round(r['avg_score'], 2)
            })
    ...
    # 5. 评分分布
    cursor.execute(f"""
        SELECT s.overall_score, COUNT(*) as count
        FROM ticket_surveys s
        JOIN tickets t ON s.ticket_id = t.id
        WHERE 1=1 {owner_filter}
        GROUP BY s.overall_score
        ORDER BY s.overall_score DESC
    """, owner_params)
    score_distribution = {str(r['overall_score']): r['count'] for r in cursor.fetchall()}
    ...
    return jsonify({
        ...
        'data': {
            ...
            'avg_overall': round(overall['avg_overall'], 2),
            'avg_response': round(overall['avg_response'], 2),
            'avg_attitude': round(overall['avg_attitude'], 2),
            'avg_quality': round(overall['avg_quality'], 2),
            'by_type': by_type,
            'by_owner': by_owner,
...
@reports_bp.route('/api/reports/erp/connections', methods=['GET'])
@token_required
...
@reports_bp.route('/api/reports/erp/connections', methods=['POST'])
@token_required
...
@reports_bp.route('/api/reports/erp/connections/<int:conn_id>', methods=['PUT'])
@token_required
...
@reports_bp.route('/api/reports/erp/connections/<int:conn_id>', methods=['DELETE'])
@token_required
...
@reports_bp.route('/api/reports/erp/connections/<int:conn_id>/test', methods=['POST'])
@token_required
...
@reports_bp.route('/api/reports/erp/sync', methods=['POST'])
@token_required
def sync_erp_data():
...
@reports_bp.route('/api/reports/erp/sync-logs', methods=['GET'])
@token_required
