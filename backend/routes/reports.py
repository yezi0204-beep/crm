...
def get_team_performance():
    ...
    if not user_can(payload['username'], 'data.view_all'):
        return jsonify({'code': 403, 'message': '权限不足，仅主任和院长可查看团队业绩', 'data': None})
...
def get_customer_satisfaction():
    ...
    owner_filter = "" if user_can(username, 'data.view_all') else "AND (t.owner_id = ? OR t.created_by = ?)"
    owner_params = [] if user_can(username, 'data.view_all') else [username, username]
